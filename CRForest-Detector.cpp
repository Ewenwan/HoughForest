...
void detect(CRForestDetector& crDetect) {
	...
	// Storage for output
	vector<vector<Mat> > vImgDetect(scales.size());
	...
	for(unsigned int i=0; i<vFilenames.size(); ++i) {
		...
		// Load image
		//IplImage *img = 0;
		Mat img = imread(impath + "/" + vFilenames[i]);
		...
		for(unsigned int k=0;k<vImgDetect.size(); ++k) {
			...
			for(unsigned int c=0;c<vImgDetect[k].size(); ++c) {
				...
				vImgDetect[k][c].create((int)(img.rows*scales[k]+0.5), (int)(img.cols*scales[k]+0.5), CV_32FC1);
			}
		}
		...
		for(unsigned int k=0;k<vImgDetect.size(); ++k) {
			...
			Mat tmp(vImgDetect[k][0].size(), CV_8UC1, Scalar::all(0));
			for(unsigned int c=0;c<vImgDetect[k].size(); ++c) {
				//cvConvertScale( vImgDetect[k][c], tmp, out_scale); //80 128
				vImgDetect[k][c] = vImgDetect[k][c] * out_scale;
...
void extract_Patches(CRPatch& Train, cv::RNG* pRNG) {
	...
	for(int i=0; i<(int)vFilenames.size(); ++i) {
		...
		double ranNum = pRNG->uniform((double)0.0, (double)1.0);
		if(subsamples_pos <= 0 || (int)vFilenames.size()<=subsamples_pos || (ranNum * double(vFilenames.size()) < double(subsamples_pos)) ) {
			// Load image
			Mat img = imread(trainpospath + "/" + vFilenames[i]);
			if(!img.data) {
	...
	for(int i=0; i<(int)vFilenames.size(); ++i) {
		...
		if(i%50==0) cout << i << " " << flush;
		double ranNum = pRNG->uniform((double)0.0, (double)1.0);
		if(subsamples_neg <= 0 || (int)vFilenames.size()<=subsamples_neg || (ranNum *double(vFilenames.size()) < double(subsamples_neg) ) ) {

			// Load image
			Mat img = imread(trainnegpath + "/" + vFilenames[i]);
			if(!img.data) {
